/*
 * Provider adattato da "StreamingCommunity" di doGior
 * (repository: doGior/doGiorsHadEnough, https://github.com/doGior/doGiorsHadEnough)
 * incluso in RetroCinema con attribuzione e grazie alla convenzione
 * di condivisione della community CloudStream.
 * Gestisce autonomamente il cambio di dominio del sito e l'estrazione
 * dei flussi VixCloud/VixSrc.
 */
package com.retrocinema.streamingcommunity
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse? {
        ...
        when (request.name) {
            ...
            "Genre" -> {
                ...
                val payload = response.body.string()
                val data =
                    tryParseJson<com.retrocinema.streamingcommunity.SearchResponse>(payload)?.data ?: return null
...
//            Log.d("$TAG:UrlFix", "Old: $url\nNew: $actualUrl")
    ...
    private suspend fun getEpisodes(props: Props): List<Episode> {
        ...
        title?.seasons?.forEach { season ->
            val responseEpisodes = emptyList<com.retrocinema.streamingcommunity.Episode>().toMutableList()
